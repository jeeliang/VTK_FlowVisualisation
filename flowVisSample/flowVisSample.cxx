...
int main(int argc, char** argv)
{
	...
	streamline1->SetInputConnection(transformFilter->GetOutputPort());
	...
	// Get the output polydata from the streamline
	vtkPolyData* outputPolyData = streamline1->GetOutput();
	...
	samplingPolyData->SetPoints(samplingPoints);

	// Create a glyph to represent the sampling points as red dots
	vtkGlyph3D* glyph3D = vtkGlyph3D::New();
	glyph3D->SetInputData(samplingPolyData);
	...
	glyphActor->GetProperty()->SetColor(1.0, 0.0, 0.0); // Set color to red
	...
	streamlineActor->GetProperty()->SetColor(1.0, 0.0, 0.0); // Set color to green
